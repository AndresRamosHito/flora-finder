...
async function fetchFeed() {
  const [feedRes, taxaRes] = await Promise.all([
    supabase
      ...
      .select("id, taxon_id, sci_name, common_name, is_sensitive, is_masked, status, location_label, observed_at, created_at, photo_url")
...
export function Feed() {
  ...
  return (
    <div className="px-4 pt-5">
      <h1 className="text-2xl font-display font-semibold tracking-tight">Avistamientos cercanos</h1>
      <p className="text-sm text-muted-foreground mt-1">Lo que la comunidad observa en la {REGION}.</p>
      ...
      <div className="mt-5 space-y-4">
        ...
        {data?.rows.map((s, i) => (
          <FeedCard key={s.id} s={s} index={i} status={s.sci_name ? data.statusBySci.get(s.sci_name) ?? null : null} />
        ))}
      </div>
    </div>
  );
...
function EmptyFeed() {
  return (
    <div className="rounded-3xl border border-dashed border-border bg-card/60 p-8 text-center">
      ...
      <p className="mt-3 text-sm text-foreground/80 font-medium">Aún no hay avistamientos en la Sierra.</p>
