...
import { Flower2, BadgeCheck, HelpCircle, MapPin, Plus, Award } from "lucide-react";
...
export const Route = createFileRoute("/lista")({
  head: () => ({ meta: [{ title: "Mi lista · OrquIDea" }, { name: "robots", content: "noindex" }] }),
  component: ListPage,
});
...
function ListPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [sRes, pRes, tRes] = await Promise.all([
        ...
        supabase.from("profiles").select("display_name, handle, points, region").eq("id", user!.id).maybeSingle(),
        supabase.from("taxa").select("id, sci_name, common_name, conservation_status, is_sensitive"),
      ]);
  ...
  if (loading || !user) return <Shell active="list"><div className="p-6 text-sm text-muted-foreground">Cargando…</div></Shell>;
      ...
      <div className="px-4 pt-5 pb-10">
        <div className="rounded-3xl bg-gradient-to-br from-leaf to-leaf/70 text-leaf-foreground p-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="font-display text-xl font-semibold">{profile?.display_name ?? "Mi lista"}</div>
              <div className="text-[11px] opacity-80 mt-1 inline-flex items-center gap-1">
                <MapPin size={11} /> {profile?.region ?? REGION}
              </div>
            </div>
            <div className="text-right">
              <div className="inline-flex items-center gap-1 text-xs opacity-90"><Award size={12} /> puntos</div>
              <div className="font-display text-3xl font-semibold leading-none">{profile?.points ?? 0}</div>
            </div>
          </div>
        ...
        {!isLoading && sightings.length === 0 && (
          <div className="mt-4 rounded-2xl border border-dashed border-border p-6 text-center">
            ...
            <Link to="/capture" className="mt-3 inline-flex items-center gap-1 rounded-full bg-orchid text-orchid-foreground px-3 py-1.5 text-xs font-semibold">
        ...
        <ul className="mt-3 space-y-3">
          {sightings.map((s) => {
            ...
            return (
              <li key={s.id} className="rounded-2xl border border-border bg-card overflow-hidden flex">
                ...
                <div className="flex-1 p-3 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="font-display italic text-sm truncate">{t?.sci_name ?? "Sin identificar"}</div>
                      <div className="text-[11px] text-muted-foreground truncate">{t?.common_name ?? "Pediremos ayuda a la comunidad"}</div>
                    </div>
                  ...
                  <div className="mt-1.5 text-[11px] text-muted-foreground flex items-center gap-2 flex-wrap">
                    <span>{new Date(s.observed_at ?? s.created_at).toLocaleDateString("es-MX")}</span>
                    <span>·</span>
                    <span className="truncate">{s.location_label ?? REGION}</span>
                  </div>
                  <div className="mt-1 text-[11px]">
                    {s.status === "verified" ? (
                      <span className="text-leaf inline-flex items-center gap-1"><BadgeCheck size={11} /> verificado</span>
                    ) : s.status === "needs_id" ? (
                      <span className="text-orchid inline-flex items-center gap-1"><HelpCircle size={11} /> necesita ID</span>
                    ) : (
